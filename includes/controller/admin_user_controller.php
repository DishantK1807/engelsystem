...
the angel is present if it is marked 'Yes'  ." . " If a 'Yes' " . " is there under active than the angel is active " . "thus has a claim on the T-shirt.". " If T-shirt contains a 'Yes' ".  "than it means angel has already received his T-shirt." . "<br /><br />\n";
    ...
    $my_highest_group = select_user_by_group($user['UID']);
    if (count($my_highest_group) > 0)
      $my_highest_group = $my_highest_group[0]['group_id'];

    $his_highest_group = select_user_by_group($id);
    ...
    if ($id != $user['UID'] && $my_highest_group <= $his_highest_group) {
      ...
      $groups = select_groups($id, $my_highest_group);
    ...
    switch ($_REQUEST['action']) {
      case 'save_groups':
        if ($id != $user['UID']) {
          $my_highest_group = select_group_by_user($user['UID']);
          $his_highest_group = select_group_by_user($id)

          if (count($my_highest_group) > 0 && (count($his_highest_group) == 0 || ($my_highest_group[0]['group_id'] <= $his_highest_group[0]['group_id']))) {
            $groups_source = select_groups($id, $my_highest_group[0]['group_id']);
            $groups = array();
            $grouplist = array();
      ...
      case 'save':
        ...
        update_group($_POST["eNick"], $_POST["eName"], $_POST["eVorname"], $_POST["eTelefon"], $_POST["eHandy"], $_POST["eAlter"], $_POST["eDECT"], $_POST["eemail"], $_REQUEST['email_shiftinfo'], $_POST["ejabber"], $_POST["eSize"], $_POST["eGekommen"], $_POST["eActive"], $force_active, $_POST["eTshirt"], $_POST["Hometown"], $id);
